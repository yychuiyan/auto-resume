...
chrome.storage.onChanged.addListener(async (changes) => {
  ...
  const handler = (e) => {
    if (e.data?.__w51 && e.data.id === cmd.id) {
      ...
      div.remove(); processing = false;
    }
  };
  window.addEventListener('message', handler);
  setTimeout(() => { if(processing){window.removeEventListener('message',handler);chrome.storage.local.set({wresult:{id:cmd.id,ok:false,msg:'超时'}});div.remove();processing=false;} }, 15000);
});
